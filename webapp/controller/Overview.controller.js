sap.ui.define([
    ...
    function (BaseController, JSONModel) {
        ...
        return BaseController.extend("materialsoverview.controller.Overview", {
            onInit: function () {
                var oViewModel = new JSONModel({
                    busy: false,
                    delay: 0,
                    oStandard: "bp_matnr,maktx",
                    oSmartTableView: "",
            ...
            onBuildSmartTable: function () {
                var oOldSmartTable = sap.ui.getCore().byId("smartTableMaterials");
                if (oOldSmartTable) {
                    oOldSmartTable.destroy();
                }

                var oDestroyedSmartTable = sap.ui.getCore().byId("smartTableMaterials");
                if (!oDestroyedSmartTable) {
                    var oView = this.getView(),
                        oModel = this.getModel("Main");

                    var oSmartTable = new sap.ui.comp.smarttable.SmartTable({
                        id: "smartTableMaterials",
                        entitySet: "xTQAxMATERIALS_DD",
                        smartFilterId: "smartFilterBarGroups",
                        tableType: "ResponsiveTable",
                        header: "{i18n>materialsOverview}",
                        showRowCount: true,
                                ...
                                if (oItems.length > 0) {
                                    ...
                                    oItems.forEach(oItem => {
                                        if (oItem instanceof sap.m.ColumnListItem) {
                                            ...
                                            oItem.attachPress(this.onPressOverViewDetail.bind(this));
                                        }
                                    });
                                }
            ...
            onPressOverViewDetail: function (oEvent) {
                sessionStorage.setItem("goToLaunchpad", "");
                this.onNavigation(oEvent.getSource().getBindingContext().getPath(), "overviewdetail", "/xTQAxMATERIALS_DD");
            },
            ...
            onSaveVariant: function (VariantName, vDefault) {
                ...
                if (this.getModel("Main").getProperty("/SmartTableBtoa")) {
                    ...
                    var oTable = sap.ui.getCore().byId("smartTableMaterials").getTable(),
                        aColumnData = [],
